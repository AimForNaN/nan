...
Unlike the guzzle response, the NaN response will attempt to use the HTTP protocol version specified by your server if none is specified, and will fall back to HTTP/1.1 as a last resort.
