...
$container = new Container([
	SomeClassInterface::class => new Entry(SomeClass::class),
]);
```
...
```php
$container[SomeClassInterface::class] = new Entry(SomeClass::class):
```
...
If an alias is not provided, the system will attempt to provide one from the entry.
...
If an alias was not provided when the entry was added, the system will be forced to loop through all entries until it finds the most compatible entry.
...
As per the same standard, you can check if an entry exists before retrieving through the `has` method.
...
$second_container = new Container([
	SomeClass::class => new Entry(SomeClass::class),
]);
...
Dependency injection will also work here, too, so long as an entry exists for the hinted types within the container.

```php
$container[SomeOtherClassInterface::class]
	= new Entry(SomeOtherClass::class);

$container[SomeClassInterface::class]
	= new Entry(SomeClass::class, ['argument', SomeOtherClass::class]);
```
...
$container[SomeClassInterface::class] = new Entry(function () {
...
```php
$container[SomeClassInterface::class]
	= new Entry(SomeClass::class, [], true);
```

```php
$container[SomeClassInterface::class]
	= new Entry(SomeClass::class, shared: true);
```

In the case where real values are used, the entries automatically become shared.

```php
$container[SomeClassInterface::class] = new Entry(new SomeClass());
