...
In NaN, dependency injection is managed by the `NaN\DI\Container` class.
...
As per the PSR-11 standard, an exception will be thrown if an entry cannot be found.
...
The relevant container can be accessed through `$this`.
